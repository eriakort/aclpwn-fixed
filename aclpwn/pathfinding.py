...
def dijkstra_find(fromid, toid, dbhost):
    # NOTE: This function uses the Neo4j REST API (/db/data/node/.../paths)
    # which was REMOVED in Neo4j 4.0. If you are running Neo4j 4.x or 5.x,
    # use --algorithm dijkstra-cypher, shortestonly, or allsimple instead.
...
def dijkstra_find_cypher(startnode, endnode, starttype='User', endtype='User'):
    # Fixed: {startnode}/{endnode} -> $startnode/$endnode (Neo4j 4.0+ param syntax)
...
def get_path(startnode, endnode, starttype='User', endtype='User', querytype='allsimple'):
    # Fixed: Result must be consumed while transaction is open (neo4j driver 5.x)
    with database.driver.session() as session:
        with session.begin_transaction() as tx:
            result = tx.run(queries[querytype] % (starttype, endtype),
                            startnode=startnode,
                            endnode=endnode)
            return list(result)  # Eagerly fetch all records before tx closes
...
def resolve_dijkstra_path(path):
    nodes = []
    rels = []
    # Fixed: {cost}/{source}/{dest} -> $cost/$source/$dest (Neo4j 4.0+ param syntax)
...
def resolve_rest_path(path):
    nodes = []
    rels = []
    # Fixed: {id} -> $id (Neo4j 4.0+ param syntax)
